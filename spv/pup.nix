...
let
  ...
  spvnode = pkgs.writeScriptBin "run.sh" ''
    ...
    # Setup DNS using resolvectl (returns `Access denied`)
    ${pkgs.systemd}/bin/resolvectl dns eth0 1.1.1.1

    # Wait until DNS resolves 'seed.multidoge.org'
    ${pkgs.bind}/bin/host -w seed.multidoge.org
...
in
{
  inherit spvnode monitor logger python3 awk;
}
